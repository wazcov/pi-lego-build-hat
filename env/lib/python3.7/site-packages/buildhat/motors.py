...
class MotorPair:
    ...
    def run_for_rotations(self, rotations, speedl=None, speedr=None, directionl=1, directionr=1):
        """Runs pair of motors for N rotations

        :param rotations: Number of rotations
        :param speedl: Speed ranging from -100 to 100
        :param speedr: Speed ranging from -100 to 100
        :param directionl: Direction either 1 or -1
        :param directionr: Direction either 1 or -1
        """
        if speedl is None:
            speedl = self.default_speed
        if speedr is None:
            speedr = self.default_speed
        self.run_for_degrees(int(rotations * 360), speedl, speedr, directionl, directionr)

    def run_for_degrees(self, degrees, speedl=None, speedr=None, directionl=1, directionr=-1):
        ...
        if directionr is -1:
            degreesr = 0 - degreesr
        th1 = threading.Thread(target=self._leftmotor._run_for_degrees, args=(degreesl, speedl))
        th2 = threading.Thread(target=self._rightmotor._run_for_degrees, args=(degreesr, speedr))
        th1.daemon = True
        th2.daemon = True
        th1.start()
        th2.start()
